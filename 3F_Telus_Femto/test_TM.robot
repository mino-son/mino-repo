...
Start(POST) Automation
    ${cmd}=    Set Variable    curl -i -H "Content-Type: application/json" -X POST "http://${TM_connection_ip}:8082/automation" -d "{\"Automationscenario\":\"test_automation_automation\",\"Command\":\"start\"}"
    ${result}=    Run Process    ${cmd}    shell=True    stdout=PIPE    stderr=PIPE    timeout=120s
    Log    ${result.stdout}
    Should Contain    ${result.stdout}    HTTP/1.1 200 OK 
    Sleep   5s
